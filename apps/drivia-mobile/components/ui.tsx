...
export function StatCard({
  icon, label, value, color = C.p, sub,
...
export function Card({
  ...
  return (
    <View style={[st.card, S.shadow, style]}>{children}</View>
  );
}

// ─── PRIMARY BUTTON ───────────────────────────────────────────────────────────
export function PrimaryBtn({
  label, onPress, loading, disabled, color = C.p, style,
}: {
  label: string; onPress: () => void; loading?: boolean;
  disabled?: boolean; color?: string; style?: ViewStyle;
}) {
  return (
    <TouchableOpacity
...
const st = StyleSheet.create({
  ...
  statCard:  { flex: 1, backgroundColor: C.white, borderRadius: R.lg, padding: 14 },
  ...
  screenSub:  { fontSize: 11, color: C.ink3, fontWeight: "500", marginBottom: 1 },
  screenTitle:{ fontSize: 20, fontWeight: "800", color: C.ink },

  card:    { backgroundColor: C.white, borderRadius: R.lg, padding: 16 },

  primaryBtn:     { height: 50, borderRadius: R.md, alignItems: "center", justifyContent: "center", paddingHorizontal: 20 },
  primaryBtnText: { fontSize: 15, fontWeight: "700", color: C.white },

  outlineBtn:     { height: 48, borderRadius: R.md, borderWidth: 1.5, alignItems: "center", justifyContent: "center", paddingHorizontal: 20 },
  outlineBtnText: { fontSize: 14, fontWeight: "700" },

  empty:     { alignItems: "center", paddingVertical: 48, paddingHorizontal: 24 },
  emptyIcon: { fontSize: 44, marginBottom: 14 },
  emptyTitle:{ fontSize: 15, fontWeight: "700", color: C.ink2, marginBottom: 6 },
  emptySub:  { fontSize: 13, color: C.ink3, textAlign: "center", lineHeight: 20 },

  divider: { height: 1, backgroundColor: C.line },

  rowItem:       { flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingVertical: 11 },
  ...
  chip:             { paddingHorizontal: 14, paddingVertical: 7, borderRadius: R.full, borderWidth: 1.5, borderColor: C.line, backgroundColor: C.white },
