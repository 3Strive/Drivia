...
const RECENT = [
  ...
  {
    id: '2',
    title: 'Weekend promo: SUVs from 18M',
    platform: 'facebook',
    reach: 810,
    enquiries: 34,
    date: 'Oct 3',
  ...
  {
    id: '3',
    title: 'Inspection-certified cars only',
    platform: 'instagram',
    reach: 540,
    enquiries: 21,
    date: 'Oct 1',
  ...
  {
    id: '4',
    title: "October deals don't miss out",
    platform: 'whatsapp',
    reach: 0,
    enquiries: 0,
    date: 'Oct 10',
...
const PLAT_LETTER: Record<string, string> = {
  whatsapp: 'W',
  facebook: 'f',
  instagram: 'ig',
};
...
export default function Broadcast() {
  ...
  const filtered =
    filter === 'All' ? RECENT : RECENT.filter((b) => b.platform === filter);
          ...
          {['All', 'whatsapp', 'facebook', 'instagram'].map((f) => {
            ...
            return (
              <TouchableOpacity
                ...
                onPress={() => setFilter(f)}
              >
                <Text style={[s.filterText, active && { color: C.white }]}>
            ...
            <Card key={b.id} style={{ marginBottom: 10 }}>
              <View style={s.bRow}>
                <View
                  style={[
                    s.platCircle,
                    { backgroundColor: PLAT_COLOR[b.platform] + '22' },
                  ]}
                >
                  <Text
                    style={[s.platLetter, { color: PLAT_COLOR[b.platform] }]}
                  >
                    {PLAT_LETTER[b.platform]}
                  </Text>
                </View>
                <View style={{ flex: 1 }}>
                ...
                <Badge
                  label={b.status}
                  color={b.status === 'Sent' ? C.green : C.gold}
                />
              </View>
              {b.status === 'Sent' && (
                <View style={s.bStats}>
                  {(
                    [
                      ...
                      ['Enquiries', String(b.enquiries)],
                      [
                        'Conv.',
                        b.reach > 0
                          ? ((b.enquiries / b.reach) * 100).toFixed(1) + '%'
                          : '0%',
                      ],
                    ] as [string, string][]
                  ).map(([k, v]) => (
...
const s = StyleSheet.create({
  screen: { flex: 1, backgroundColor: C.bg, gap: 10 },
  composeBtn: {
    backgroundColor: C.p,
    borderRadius: 10,
