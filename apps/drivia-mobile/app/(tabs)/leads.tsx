    ...
    >
      <View style={s.leadTop}>
        ...
        <View
          style={[
            s.sourcePill,
            { backgroundColor: SOURCE_COLOR[lead.source] + '22' },
          ]}
        >
          <Text style={[s.sourceText, { color: SOURCE_COLOR[lead.source] }]}>
            {lead.source}
          </Text>
        </View>
      </View>
      <View style={s.leadBottom}>
        ...
        <View style={{ flexDirection: 'row', gap: 8 }}>
          <TouchableOpacity
            style={[s.actionBtn, { backgroundColor: C.wa + '18' }]}
          >
            <Text style={{ fontSize: 14 }}>💬</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[s.actionBtn, { backgroundColor: C.pLight }]}
          >
            <Text style={{ fontSize: 14 }}>📞</Text>
          </TouchableOpacity>
        </View>
    ...
    <View>
      ...
      >
        {[
          ...
          {
            label: 'Active',
            val: leads.filter((l) => !['Sold', 'Lost'].includes(l.status))
              .length,
            color: C.green,
          },
          ...
          <View
            ...
            style={[
              s.summaryPill,
              {
                backgroundColor: s2.color + '12',
                borderColor: s2.color + '30',
              },
            ]}
          >
            <Text style={[s.summaryVal, { color: s2.color }]}>{s2.val}</Text>
            <Text style={[s.summaryLabel, { color: s2.color }]}>
              {s2.label}
            </Text>
          </View>
        ))}
      ...
      >
        {STAGES.map((stage) => {
          ...
          return (
            ...
            >
              <Text style={[s.stageTabText, active && { color: C.white }]}>
                {stage}
              </Text>
              <View
                style={[
              ...
              >
                <Text
                  style={[
                    s.stageBadgeText,
                    { color: active ? C.white : STAGE_COLOR[stage] },
                  ]}
                >
                  {stageCounts[stage]}
                </Text>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={s.empty}>
            ...
            <Text style={{ fontSize: 15, fontWeight: '700', color: C.ink2 }}>
          ...
          <View style={s.modalSheet}>
            ...
            <View style={s.modalHeader}>
              ...
              <Badge
                ...
                color={STAGE_COLOR[selected.status]}
              />
            </View>
                ...
                {STAGES.filter((st) => st !== selected.status).map((st) => (
                  <TouchableOpacity
                    ...
                    style={[
                      s.moveBtn,
                      {
                        backgroundColor: STAGE_COLOR[st] + '18',
                        borderColor: STAGE_COLOR[st] + '44',
                      },
                    ]}
                    onPress={() => moveLead(selected.id, st)}
                  >
                    <Text style={[s.moveBtnText, { color: STAGE_COLOR[st] }]}>
                      {st}
                    </Text>
                  </TouchableOpacity>
                ))}
            ...
            <View style={s.modalActions}>
              <TouchableOpacity
                style={[s.actionLarge, { backgroundColor: C.wa }]}
              >
                <Text style={s.actionLargeText}>💬 WhatsApp</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[s.actionLarge, { backgroundColor: C.pLight }]}
              >
