...
const RECENT_SALES = [
  { id:"1", car:"2021 Toyota Camry",   buyer:"Chidi Okafor",   price:16500000, date:"Nov 1",  platform:"WhatsApp" },
  { id:"2", car:"2018 Lexus RX350",    buyer:"Biodun Fashola", price:26000000, date:"Oct 28", platform:"Referral" },
  { id:"3", car:"2020 Honda CR-V",     buyer:"Ada Nwosu",      price:20500000, date:"Oct 18", platform:"Facebook" },
  { id:"4", car:"2019 Mercedes C300",  buyer:"Kola Adesanya",  price:34000000, date:"Sep 30", platform:"Walk-in"  },
];
const PLAT_COLOR: Record<string,string> = { WhatsApp:C.wa, Facebook:C.fb, Instagram:C.ig, "Walk-in":"#805AD5", Referral:"#D69E2E" };
...
function Toggle({ on, onChange }: { on:boolean; onChange:()=>void }) {
  return (
    <TouchableOpacity onPress={onChange} style={[ts.track, on && ts.trackOn]} activeOpacity={0.9}>
      <View style={[ts.thumb, on && ts.thumbOn]} />
    </TouchableOpacity>
  );
}
const ts = StyleSheet.create({
  track:   { width:38, height:20, borderRadius:10, backgroundColor:"#E2E8F0", justifyContent:"center", paddingHorizontal:2 },
  trackOn: { backgroundColor:C.p },
  thumb:   { width:16, height:16, borderRadius:8, backgroundColor:"#fff", shadowColor:"#000", shadowOpacity:0.15, shadowRadius:2, shadowOffset:{width:0,height:1} },
      ...
      <View style={s.banner}>
        ...
        <View style={s.bannerContent}>
          <View style={s.avatarWrap}>
            <View style={s.avatar}><Text style={s.avatarText}>LA</Text></View>
          </View>
          <View style={{ flex:1 }}>
        ...
        <View style={s.statStrip}>
          {[{val:"47",label:"Listings"},{val:"312",label:"Cars Sold"},{val:"4.8",label:"Rating"},{val:"2019",label:"Since"}].map((st,i) => (
            <View key={st.label} style={[s.statItem, i < 3 && s.statItemBorder]}>
      ...
      <ScrollView style={s.body} showsVerticalScrollIndicator={false}>
        ...
        {tab === "overview" && (
          <View style={s.section}>
            ...
            <Card>
              {[{label:"Business Name",val:name},{label:"Phone",val:phone},{label:"Location",val:loc}].map((f,i) => (
                <View key={f.label}>
            ...
            <SectionHeader title="Performance" style={{ marginTop:20 }} />
            <Card>
              {[
                {label:"Total Revenue",      val:"₦318M",  color:C.p    },
                {label:"Cars Sold",          val:"312",    color:C.green},
                {label:"Avg. Selling Price", val:"₦21.4M", color:"#2C7A7B"},
            ...
            <Card>
              {[
                {plat:"WhatsApp", handle:"08012345678",     color:C.wa, connected:true },
                {plat:"Facebook", handle:"@LagosAutoHub",   color:C.fb, connected:true },
                {plat:"Instagram",handle:"@lagos.auto.hub", color:C.ig, connected:true },
                {plat:"Twitter",  handle:"Not connected",   color:C.tw, connected:false},
              ].map((soc,i) => (
                <View key={soc.plat}>
                  <View style={s.socRow}>
                    <View style={[s.socIcon, { backgroundColor:soc.color+"22", opacity:soc.connected?1:0.4 }]}>
                      <Text style={[s.socLetter, { color:soc.color }]}>{soc.plat.charAt(0)}</Text>
                    </View>
                    <View style={{ flex:1 }}>
                      <Text style={s.socName}>{soc.plat}</Text>
                      <Text style={s.socHandle}>{soc.handle}</Text>
                    </View>
                    <Badge label={soc.connected?"Connected":"Connect"} color={soc.connected?C.green:C.ink3} />
                  </View>
                  {i < 3 && <Divider />}
                </View>
              ))}
        ...
        {tab === "sales" && (
          <View style={s.section}>
            <View style={s.saleSummary}>
              {[
                {icon:"💰",label:"Revenue This Year",  val:"₦218M",  color:C.p    },
                {icon:"🚗",label:"Cars Sold This Year", val:"21",     color:C.green},
                {icon:"💬",label:"From WhatsApp",       val:"61%",    color:C.wa   },
                {icon:"📊",label:"Avg. Sale Price",     val:"₦21.4M", color:"#2C7A7B"},
              ].map(st => (
                <View key={st.label} style={[s.saleStat, { borderColor:st.color+"30", backgroundColor:st.color+"0A" }]}>
                  <Text style={{ fontSize:18, marginBottom:4 }}>{st.icon}</Text>
                  <Text style={[s.saleStatVal, { color:st.color }]}>{st.val}</Text>
                ...
                <View key={sale.id}>
                  <View style={s.saleRow}>
                    ...
                    <View style={{ flex:1 }}>
                      ...
                      <Text style={s.saleBuyer}>{sale.buyer} · {sale.date}</Text>
                    </View>
                    <View style={{ alignItems:"flex-end", gap:4 }}>
                      <Text style={s.salePrice}>₦{(sale.price/1e6).toFixed(1)}M</Text>
                      <View style={[s.platDot, { backgroundColor:PLAT_COLOR[sale.platform]??"#888" }]} />
                    </View>
                  </View>
        ...
        {tab === "settings" && (
          <View style={s.section}>
            ...
            <Card>
              {[
                {label:"Change Password",   sub:"Last changed 90 days ago", danger:false},
                {label:"Two-Factor Auth",   sub:"Currently disabled",        danger:false},
                {label:"Connected Devices", sub:"2 active sessions",          danger:false},
                {label:"Delete Account",    sub:"This cannot be undone",      danger:true },
              ].map((item,i) => (
                <View key={item.label}>
                  <TouchableOpacity style={s.acctRow}>
                    <View style={{ flex:1 }}>
                      <Text style={[s.acctLabel, item.danger && { color:C.red }]}>{item.label}</Text>
            ...
            <TouchableOpacity style={s.logoutBtn} onPress={() => router.replace("/(auth)")}>
...
const s = StyleSheet.create({
  ...
  banner:        { backgroundColor:C.p, paddingTop:52, paddingHorizontal:20, paddingBottom:0 },
  bannerGrad:    { position:"absolute", inset:0, opacity:0.3 },
  bannerContent: { flexDirection:"row", alignItems:"center", gap:14, marginBottom:16 },
  avatarWrap:    {},
  avatar:        { width:60, height:60, borderRadius:30, backgroundColor:"rgba(255,255,255,0.25)", alignItems:"center", justifyContent:"center", borderWidth:3, borderColor:"rgba(255,255,255,0.4)" },
  ...
  statStrip:     { flexDirection:"row", backgroundColor:"rgba(0,0,0,0.15)", borderRadius:0, marginHorizontal:-20, marginTop:0 },
  ...
  tabRow:        { flexDirection:"row", backgroundColor:C.white, borderBottomWidth:1, borderBottomColor:C.line },
  ...
  infoField:     { flexDirection:"row", justifyContent:"space-between", alignItems:"center", paddingVertical:12 },
  ...
  socRow:        { flexDirection:"row", alignItems:"center", gap:10, paddingVertical:10 },
  socIcon:       { width:32, height:32, borderRadius:9, alignItems:"center", justifyContent:"center" },
  socLetter:     { fontSize:14, fontWeight:"900" },
  socName:       { fontSize:13, fontWeight:"600", color:C.ink },
  socHandle:     { fontSize:11, color:C.ink3 },
  saleSummary:   { flexDirection:"row", flexWrap:"wrap", gap:10, marginBottom:20 },
  ...
  carIcon:       { width:34, height:34, borderRadius:9, backgroundColor:C.pLight, alignItems:"center", justifyContent:"center" },
  ...
  logoutBtn:     { marginTop:20, height:50, borderRadius:14, borderWidth:1.5, borderColor:C.red, alignItems:"center", justifyContent:"center" },
