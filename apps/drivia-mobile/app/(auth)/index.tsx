...
import {
  ...
  ScrollView, FlatList, Dimensions, KeyboardAvoidingView, Platform,
} from "react-native";
...
// ─── TYPES ────────────────────────────────────────────────────────────────────
interface PlanOption  { id: string; name: string; price: string; period: string; popular: boolean; features: string[]; }
interface PlatOption  { id: string; name: string; letter: string; color: string; desc: string; }

const PLANS: PlanOption[] = [
  { id:"free", name:"Free",  price:"₦0",   period:"forever",   popular:false, features:["5 listings","10 broadcasts","2 platforms","Basic leads"] },
  { id:"pro",  name:"Pro",   price:"₦15K", period:"per month", popular:true,  features:["30 listings","200 broadcasts","3 platforms","Full CRM"] },
  { id:"vvip", name:"VVIP",  price:"₦45K", period:"per month", popular:false, features:["Unlimited","Unlimited","All platforms","Manager"] },
];

const PLATFORMS: PlatOption[] = [
  { id:"whatsapp",  name:"WhatsApp",  letter:"W",  color:C.wa, desc:"Business messaging" },
  { id:"facebook",  name:"Facebook",  letter:"f",  color:C.fb, desc:"Page + Marketplace" },
  { id:"instagram", name:"Instagram", letter:"ig", color:C.ig, desc:"Photo listings"     },
  { id:"twitter",   name:"Twitter/X", letter:"X",  color:C.tw, desc:"Reach & visibility" },
];

const CAR_TYPES = ["Brand New","Tokunbo","Nigerian Used","SUVs","Saloon Cars","Trucks","Luxury","Budget"];
  ...
  return (
    <View style={s.progressRow}>
      {Array.from({ length: TOTAL }).map((_, i) => (
        <View key={i} style={[
          s.dot,
          i + 1 === step ? s.dotActive : i + 1 < step ? s.dotDone : s.dotPending,
        ]} />
      ))}
    </View>
  );
  ...
  return (
    <View style={s.step}>
      ...
      <View style={s.inputRow}>
        <View style={s.flag}><Text style={{ fontSize: 20 }}>🇳🇬</Text></View>
        <TextInput style={[s.input, { flex: 1, borderLeftWidth: 0, borderRadius: 0, borderTopRightRadius: 12, borderBottomRightRadius: 12 }]}
          placeholder="0801 234 5678" keyboardType="phone-pad"
      ...
      <TouchableOpacity onPress={onNext}>
        <Text style={s.linkText}>I'll use email instead →</Text>
      </TouchableOpacity>
    </View>
  );
}
...
function Step2({ onNext, onBack }: { onNext: () => void; onBack: () => void }) {
  ...
  return (
    <View style={s.step}>
      ...
      <Text style={s.sub}>We sent a 4-digit code to <Text style={{ fontWeight:"700", color:C.ink }}>0801 234 5678</Text></Text>

      <View style={s.otpRow}>
        {otp.map((v, i) => (
          <TextInput key={i} ref={refs[i]} style={[s.otpBox, v ? s.otpBoxFilled : null]}
            value={v} onChangeText={t => handleOtp(t, i)}
      ...
      <View style={s.navRow}>
        ...
        <TouchableOpacity style={[s.nextBtn, { flex: 1 }]} onPress={onNext} activeOpacity={0.85}>
    ...
    <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false}>
      <View style={s.step}>
        ...
        <View style={s.logoUpload}>
          ...
          <Text style={[s.hint, { color: C.p, marginTop: 4 }]}>Upload Logo</Text>
        </View>

        <Text style={s.label}>Business Name *</Text>
        <TextInput style={s.input} placeholder="e.g. Lagos Auto Hub" value={biz} onChangeText={setBiz} />
        ...
        <View style={{ flexDirection: "row", gap: 10 }}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <TextInput style={s.input} placeholder="2019" keyboardType="number-pad" value={year} onChangeText={setYear} />
          </View>
        </View>
        ...
        <TextInput style={[s.input, s.textarea]}
          ...
          value={about} onChangeText={setAbout} multiline numberOfLines={3} />
...
function Step4({ onNext, onBack }: { onNext: () => void; onBack: () => void }) {
  const [selectedTypes, setSelectedTypes] = useState<string[]>(["Brand New","Tokunbo"]);
  ...
  return (
    <View style={s.step}>
      ...
      {PLATFORMS.map(pl => {
        ...
        return (
          <TouchableOpacity key={pl.id} onPress={() => toggle(pl.id)}
      ...
      <View style={s.tipBox}>
        ...
        <Text style={s.tipSub}>From Settings → Manage Socials at any time.</Text>
      </View>
    ...
    <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false}>
      <View style={s.step}>
          ...
          return (
            <TouchableOpacity key={plan.id} onPress={() => setSelected(plan.id)}
              ...
              <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start" }}>
                ...
                {sel && <View style={s.checkCircle}><Text style={{ color: C.white, fontSize: 10, fontWeight: "800" }}>✓</Text></View>}
              </View>
              <View style={{ marginTop: 10, gap: 4 }}>
                {plan.features.map(f => (
                  <Text key={f} style={s.planFeat}>✓  {f}</Text>
                ))}
              </View>
            </TouchableOpacity>
          );
        ...
        <View style={[s.tipBox, { backgroundColor: C.pLight }]}>
          <Text style={[s.tipTitle, { color: C.p }]}>🎁 14-day free trial on Pro</Text>
          <Text style={[s.tipSub, { color: C.ink3 }]}>Try all Pro features. Cancel before day 14.</Text>
        </View>
...
function Step7() {
  ...
  return (
    <View style={[s.step, { alignItems: "center" }]}>
      ...
      <Text style={[s.title, { textAlign: "center" }]}>Welcome to Drivia 🚗</Text>
      <Text style={[s.sub, { textAlign: "center" }]}>Your dealer account is ready.</Text>

      <View style={{ width: "100%", gap: 10, marginBottom: 28 }}>
        {checks.map(c => (
          <View key={c.label} style={s.checkRow}>
            <View style={s.checkIconBox}><Text>{c.icon}</Text></View>
      ...
      <TouchableOpacity style={[s.nextBtn, { width: "100%" }]}
        onPress={() => router.replace("/(tabs)")} activeOpacity={0.85}>
        <Text style={s.nextBtnText}>Go to my Dashboard →</Text>
      </TouchableOpacity>

      <TouchableOpacity style={{ marginTop: 14 }}
        onPress={() => router.replace("/(tabs)/inventory")}>
        <Text style={[s.linkText, { color: C.ink3 }]}>+ Add my first listing now</Text>
      </TouchableOpacity>
    </View>
  );
}
...
export default function Onboarding() {
  ...
  const steps = [
    ...
    <Step5 onNext={next} onBack={back} />,
    <Step6 onNext={next} onBack={back} />,
    <Step7 />,
  ];
...
const s = StyleSheet.create({
  ...
  logo:          { width: 32, height: 32, borderRadius: 9, backgroundColor: C.p, alignItems: "center", justifyContent: "center" },
  ...
  progressTrack: { height: 3, backgroundColor: C.line, borderRadius: 2, overflow: "hidden", marginBottom: 10 },
  ...
  eyebrow:       { fontSize: 10, fontWeight: "700", letterSpacing: 1.4, textTransform: "uppercase", color: C.p, marginBottom: 8 },
  title:         { fontSize: 24, fontWeight: "800", color: C.ink, lineHeight: 30, marginBottom: 6 },
  sub:           { fontSize: 14, color: C.ink3, lineHeight: 22, marginBottom: 22 },
  label:         { fontSize: 12, fontWeight: "700", color: C.ink2, marginBottom: 6, marginTop: 4 },
  hint:          { fontSize: 11, color: C.ink3, marginTop: 5 },

  inputRow:      { flexDirection: "row" },
  flag:          { width: 48, height: 46, backgroundColor: C.white, borderWidth: 1.5, borderColor: C.line, borderRadius: 12, borderTopRightRadius: 0, borderBottomRightRadius: 0, alignItems: "center", justifyContent: "center" },
  input:         { height: 46, borderRadius: 12, borderWidth: 1.5, borderColor: C.line, backgroundColor: C.white, paddingHorizontal: 14, fontSize: 14, color: C.ink, marginBottom: 4 },
  textarea:      { height: 88, paddingTop: 12, textAlignVertical: "top" },

  otpRow:        { flexDirection: "row", gap: 12, marginBottom: 24 },
  otpBox:        { flex: 1, height: 58, borderRadius: 14, borderWidth: 2, borderColor: C.line, backgroundColor: C.white, fontSize: 26, fontWeight: "800", color: C.p, textAlign: "center" },
  otpBoxFilled:  { borderColor: C.p },

  navRow:        { flexDirection: "row", gap: 10, marginTop: 20 },
  backBtn:       { height: 50, paddingHorizontal: 18, borderRadius: 14, borderWidth: 2, borderColor: C.line, backgroundColor: C.white, alignItems: "center", justifyContent: "center" },
  backBtnText:   { fontSize: 14, fontWeight: "700", color: C.ink2 },
  nextBtn:       { height: 50, borderRadius: 14, backgroundColor: C.p, alignItems: "center", justifyContent: "center" },
  nextBtnText:   { fontSize: 14, fontWeight: "700", color: C.white },
  linkText:      { fontSize: 13, fontWeight: "600", color: C.p, textAlign: "center" },

  waBtn:         { height: 50, borderRadius: 13, backgroundColor: C.wa, alignItems: "center", justifyContent: "center", marginBottom: 16 },
  ...
  chip:          { paddingHorizontal: 13, paddingVertical: 8, borderRadius: 10, borderWidth: 1.5, borderColor: C.line, backgroundColor: C.white },
  ...
  platCard:         { flexDirection: "row", alignItems: "center", gap: 12, padding: 14, borderRadius: 12, borderWidth: 2, borderColor: C.line, backgroundColor: C.white, marginBottom: 10 },
  platCardSelected: { borderColor: C.p, backgroundColor: C.pLight },
  platIcon:         { width: 36, height: 36, borderRadius: 10, alignItems: "center", justifyContent: "center" },
  platLetter:       { fontSize: 13, fontWeight: "900" },
  platName:         { fontSize: 13, fontWeight: "700", color: C.ink },
  platDesc:         { fontSize: 11, color: C.ink3 },

  planCard:         { borderRadius: 14, borderWidth: 2, borderColor: C.line, backgroundColor: C.white, padding: 16, marginBottom: 10, overflow: "hidden" },
  ...
  popularBadge:     { position: "absolute", top: 0, left: 0, right: 0, backgroundColor: C.p, paddingVertical: 4, alignItems: "center" },
  popularText:      { fontSize: 9, fontWeight: "800", color: C.white, letterSpacing: 1 },

  tipBox:    { backgroundColor: "#FFFAF0", borderRadius: 11, padding: 12, marginVertical: 14, borderWidth: 1, borderColor: "#FAF089" },
  tipTitle:  { fontSize: 12, fontWeight: "700", color: "#744210", marginBottom: 2 },
  tipSub:    { fontSize: 11, color: "#92400E", lineHeight: 16 },

  checkCircle:  { width: 20, height: 20, borderRadius: 6, backgroundColor: C.p, alignItems: "center", justifyContent: "center" },
  successRing:  { width: 88, height: 88, borderRadius: 44, backgroundColor: C.p, alignItems: "center", justifyContent: "center", marginBottom: 20, shadowColor: C.p, shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.4, shadowRadius: 20, elevation: 10 },
  checkRow:     { flexDirection: "row", alignItems: "center", gap: 12, backgroundColor: C.white, borderRadius: 12, padding: 12, borderWidth: 1, borderColor: C.line },
  checkIconBox: { width: 30, height: 30, borderRadius: 8, backgroundColor: C.greenBg, alignItems: "center", justifyContent: "center" },
  checkLabel:   { flex: 1, fontSize: 13, fontWeight: "600", color: C.ink },
  checkDone:    { fontSize: 12, fontWeight: "700", color: C.green },

  logoUpload:    { width: 72, height: 72, borderRadius: 36, backgroundColor: C.pLight, borderWidth: 2, borderColor: C.p, borderStyle: "dashed", alignItems: "center", justifyContent: "center", marginBottom: 16, alignSelf: "flex-start" },
});
